...
class DecayModule(nn.Module):
	...
	def forward(self, input_, hx = None):
		...
		w_w = (self.sigmoid(self.rgate) * hx) + ((1-self.sigmoid(self.rgate)) * (w_x + w_h))
...
class DECAY_RNN(nn.Module):


	def __init__(self, input_size, hidden_size, num_layers = 1, dropout=0., activation='relu', nodiag=False, recurrent_dropout=0.):
		super(DECAY_RNN, self).__init__()
	...
	def forward_single_layer(self, input_, layer, h_0=None, max_time=50):
		...
		if self.recurrent_dropout!=0:
			mask =  torch.ones(M, self.hidden_size)*(1-self.recurrent_dropout)
		...
		for time in range(max_time):
			...
			if self.training and self.recurrent_dropout!=0:
				state = (state*mask)/(1-self.recurrent_dropout)
