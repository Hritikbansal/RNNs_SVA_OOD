...
parser.add_argument('--prop_train', type=float, default=0.1,
					help='amount of training data.')
parser.add_argument('--validation_size', type=int, default=7000,
	...
	if train:
		...
		for l in [1]:#, 2, 3, 4, 5]:
			...
			pvn.pipeline(False,train_bsz=128, test_bsz= 2048, model="decay_fullGram.pkl", load_data=True,load=True,num_epochs=10, model_prefix='decay_fullGram', 
						data_name='fullGram', test_size=0, lr=0.01, annealing=True,act_attention=False,domain_adaption=args.domain_adaption,test_demarcated=args.test_demarcated,demarcate_train=args.demarcate_train, L=l, K=5, augment_train=args.augment_train, augment_test=args.augment_test, verb_embedding=args.verb_embedding)

else:
	pvn = PredictVerbNumber(args.model, filenames.deps, embedding_size=50, hidden_size=50, output_size=2, prop_train=0.1, output_filename='output_log.txt')
