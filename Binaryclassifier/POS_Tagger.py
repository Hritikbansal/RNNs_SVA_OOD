...
class Agreement_model(nn.Module):
	...
	def __init__(self, rnn_arch, embedding_size, hidden_size, vocab_size, num_layers, output_size, dropout=0., emb_attention=False, act_attention=False, max_attn=False, num_heads=1, activation='relu'):
		...
		# No use of attention in POS Tagging
		# if emb_attention:
	...
	def predict(self, input_, ground_truth, hidden=None, compute_loss=False):
		...
		if self.rnn_arch== 'LSTM' or self.rnn_arch =='ONLSTM':
			output, h_last, (h_n, c_n) = self.forward(input_, hidden) # output is (bsz, time_stamps, hidden_size) and we need to take final time stamp value 
		else:
			output, h_last, h_n = self.forward(input_, hidden)

		ground_truth = ground_truth.reshape(input_bsz,max_len)
		prediction = torch.argmax(output, dim= -1).view(input_bsz, max_len)

		acc  = torch.sum(torch.eq(prediction, ground_truth))  # this accuracy is just the number of correct examples!!!  WATCH OUT! 
		# note that, in this case the accuracy would be per word as well. Total samples will not be bsz, but it will be bsz*max_len

		if compute_loss:
			loss = loss_func(output, ground_truth)
			# if self.act_attention or self.max_attn:
			# 	return loss, output, prediction, acc, attention_weights[:, -1, :], h_n
			# else:
			return loss, output, prediction, acc, h_n
		else:
			# if self.act_attention or self.max_attn:
			# 	return output, prediction, acc, attention_weights[:, -1, :], h_n
			# else:
			return output, prediction, acc, h_n
