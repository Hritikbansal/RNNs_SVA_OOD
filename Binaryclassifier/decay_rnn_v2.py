...
class DecayModule(nn.Module):
	...
	def __init__(self, input_size , hidden_size, bias = True, num_chunks = 1, activation='relu'):
		...
		self.rgate = nn.Parameter(1.4*torch.ones((hidden_size, 1)), requires_grad=True)
	...
	def reset_parameters(self):
		...
		for name,param in self.named_parameters():
			if name=="rgate":
				param.data  = 1.4*torch.ones((self.hidden_size, 1))
	...
	def forward(self, input_, hx = None):
		...
		w_w = (self.Sigmoid(self.rgate) * hx.t()).t() + ((1-self.sigmoid(self.rgate)) * (w_x + w_h).t()).t()
...
class GATED_DECAY_RNN(nn.Module):


	def __init__(self, input_size, hidden_size, num_layers = 1, dropout=0.2, activation='relu'):
		super(GATED_DECAY_RNN, self).__init__()
		...
		for layer in range(num_layers):
			...
			cell = DecayModule(input_size  = layer_input_size, hidden_size = self.hidden_size,activation=self.activation)
