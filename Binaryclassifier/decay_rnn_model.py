...
import POS_Tagger
from utils import deps_from_tsv, deps_to_tsv, dump_template_waveforms
from utils import gen_inflect_from_vocab
import time
...
class Demarcated_dataset(Dataset):
	...
	def __init__(self, x_train, y_train, dep_train):
		super(Demarcated_dataset, self).__init__()
		self.x_train = x_train
		self.y_train = y_train
		self.dep_train = dep_train
		assert (x_train).shape[0] == (y_train).shape[0]
		self.length = (x_train).shape[0]

	def __getitem__(self, index):
		return self.x_train[index], self.y_train[index], self.dep_train[index]
...
class DECAY_RNN_Model(object):
	...
	def __init__(self, rnn_arch, filename=None, embedding_size=50, hidden_size=50, output_size=10, num_layers=1,
				 dropout=0.2,
	...
	def create_train_test_dataloader(self):
		...
		verb_loc_train = torch.stack([torch.tensor(elem["verb_index"]) for elem in self.deps_train]).to(
			device).squeeze()
		self.Train_DataGenerator = DataLoader(BatchedDataset(x_train, y_train, verb_loc_train),
											  batch_size=self.train_bsz, shuffle=True, drop_last=False)
		...
		self.Test_DataGenerator = DataLoader(BatchedDataset(x_test, y_test, verb_loc_test), batch_size=self.test_bsz,
	...
	def create_train_test_dataloader_POS(self):
		...
		verb_loc_train = torch.stack([torch.tensor(elem["verb_index"]) for elem in self.deps_train]).to(
			device).squeeze()
		self.Train_DataGenerator = DataLoader(BatchedDataset(x_train, pos_train, verb_loc_train),
											  batch_size=self.train_bsz, shuffle=True, drop_last=False)
		...
		self.deps_test = list(self.deps_test)
		verb_loc_test = torch.stack([torch.tensor(elem["verb_index"]) for elem in self.deps_test]).to(device).squeeze()
		self.Test_DataGenerator = DataLoader(BatchedDataset(x_test, pos_test, verb_loc_test), batch_size=self.test_bsz,
											 shuffle=True, drop_last=False)
	...
	def create_demarcated_dataloader(self):
		...
		if hasattr(self, 'testing_dict'):
			for key in self.testing_dict.keys():
				x_test, y_test, dep_test = zip(*self.testing_dict[key])
				x_test = torch.tensor(list(x_test), dtype=torch.long).to(device)
				y_test = torch.tensor(list(y_test)).to(device)
				testing_demarcated_dataloader[key] = DataLoader(Demarcated_dataset(x_test, y_test, dep_test),
																batch_size=self.test_bsz, shuffle=False, drop_last=False)
		...
		if hasattr(self, 'training_dict'):
			for key in self.training_dict.keys():
				x_train, y_train, dep_train = zip(*self.training_dict[key])
				x_train = torch.tensor(list(x_train), dtype=torch.long).to(device)
				y_train = torch.tensor(list(y_train)).to(device)
				training_demarcated_dataloader[key] = DataLoader(Demarcated_dataset(x_train, y_train, dep_train),
																 batch_size=self.test_bsz, shuffle=True,
																 drop_last=False)
	...
	def create_OOD_dataloader(self, final_dict_testing):
		...
		for key in final_dict_testing.keys():
			...
			if len(x) == 0: continue
			ood_dataloader_dict[key] = DataLoader(ood_dataset(x_sent, x, y), batch_size=self.train_bsz, shuffle=True,
												  drop_last=False)
	...
	def demark_testing(self):
		...
		print("Demarcating Test Set")
		for i in (range(len(X_test))):
			verb = deps_test[i]['verb']
			if verb not in self.vocab_to_ints:
				count_notinvocab += 1
				# print("t_verb ",t_verb, len(t_verb),sep=" ")
				continue

			key0 = deps_test[i]['n_intervening']
			key1 = deps_test[i]["n_diff_intervening"]
			...
			key = key1  # comment this and uncomment below for interveners x attractors

			# key = key01

			if not key in testing_dict.keys():
				testing_dict[key] = []
			testing_dict[key].append((X_test[i], Y_test[i], deps_test[i]))
		...
		for i in (range(len(X_train))):
			...
			if not key in training_dict.keys():
				training_dict[key] = []
			training_dict[key].append((X_train[i], Y_train[i], deps_train[i]))
	...
	def result_demarcated(self):
		...
		dependency_fields = ['sentence', 'orig_sentence', 'pos_sentence',
							 ...
							 'max_depth', 'all_nouns', 'nouns_up_to_verb', 'label']
		...
		with torch.no_grad():
			for key in loader_dict.keys():
				...
				for x_test, y_test, dep_test in loader:
					bsz = x_test.size(0)
					x_test = x_test.view(bsz, self.maxlen)
					y_test = y_test.view(bsz, )
					example_processed += bsz
					if self.act_attention or self.max_attn:
						_, pred, _, acc, attention_weights, h_n, h_last = self.model.predict(x_test, y_test,
																							 compute_loss=True)
					else:
						_, incorr_indices, pred, _, acc, h_n, h_last = self.model.predict(x_test, y_test, compute_loss=True)
					correct += acc
					hidden_state_dict[key].append((h_n, pred, y_test))

					#print(len(dep_test.keys()))
					#print(dep_test)

					#for kei in dep_test:
					#	for i in incorr_indices:
					#		incorrect[kei].append(dep_test[kei][i])

					for incorr_index in incorr_indices:
						temp = {}
						for kei in dep_test:
							val = dep_test[kei][incorr_index]
							if kei in ["subj_index","verb_index" , "n_intervening" , "n_diff_intervening" , "distance" , "max_depth"]:
								temp[kei] = int(val)
							else:
								temp[kei] = val
		...
		print("no. of incorrect predictions: ",len(incorrect))
		#print(incorrect)


		writer = csv.writer(open("incorr.tsv", 'w'), delimiter='\t')
	...
	def create_model(self):
		...
		if not self.train_tagger:
			self.model = Agreement_model(rnn_arch=self.rnn_arch, embedding_size=self.embedding_size,
										 hidden_size=self.hidden_size, vocab_size=len(self.vocab_to_ints) + 1,
										 num_layers=self.num_layers, output_size=self.output_size, dropout=self.dropout,
										 act_attention=self.act_attention, max_attn=self.max_attn,
										 num_heads=self.nheads, activation=self.activation)
		else:
			self.model = POS_Tagger.Agreement_model(rnn_arch=self.rnn_arch, embedding_size=self.embedding_size,
													hidden_size=self.hidden_size,
													vocab_size=len(self.vocab_to_ints) + 1, num_layers=self.num_layers,
		...
		for i in range(len(ex_list)):
			for keys in ex_list[i][0].keys():
				...
				if len(list1[0]) > 2:  # ignoring the 3 tuples in the templates
					continue
				if (ex_list[i][1],
					keys) in test_example.keys():  # this just means if (file, singular) is a key to test_example dictionary, if not then initialize the key
					pass
				else:
	...
	def valid_input(self, external_testing_dict):
		...
		for keys in external_testing_dict.keys():
			...
			assert len(x_sentences) == len(x) == len(
				y), "assert failed! length of sentences is different from length of actual testing sentence per template per sing/plur"
		return final_dict_testing

	def pipeline(self, train, train_bsz=128, test_bsz=32, load=False, model='', test_size=7000, model_prefix='_',
				 num_epochs=20, load_data=False, save_data=False, test_linzen_template_pvn=False,
				 linzen_template_filename=None,
				 train_size=None, data_name='Not', lr=0.001, annealing=False, nheads=1, activation='relu',
				 act_attention=False,
				 max_attn=False, use_hidden=False, K=5, L=1, train_tagger=False, compare_models=False, m1=None, m2=None,
				 domain_adaption=False, intermediate=False, domain_adaption2=False, filen=None, test_demarcated=False,
				 demarcate_train=False, ood=False, augment_train=True,
				 augment_test=True, augment_ratio=1, verb_embedding=False):
		...
		self.domain_adaption2 = domain_adaption2
		...
		if not save_data:
			...
			print("Loading Data!")
			self.load_train_and_test(test_size, data_name, self.domain_adaption, self.intermediate,
									 self.domain_adaption2)
		else:
			self.log('creating data')
			print("Creating data")
			# examples = self.load_examples(data_name, save_data, None if train_size is None else train_size*10, self.filen)

			# Generating vocab to int, pos to int files and test file only once (While generating for the natural case)
			if self.filen == "train_nat.tsv":
				self.vocab_ints(data_name, self.save_data, n_examples=None)

				examples_test = self.load_examples(data_name, "test.tsv", None)
				self.create_train_and_test(examples_test, data_name, self.save_data, dtype="test",
										   domain_adaption=self.domain_adaption, intermediate=self.intermediate,
										   domain_adaption2=self.domain_adaption2)

			examples_train = self.load_examples(data_name, self.filen, None)
			self.create_train_and_test(examples_train, data_name, self.save_data, dtype="train",
									   domain_adaption=self.domain_adaption, intermediate=self.intermediate,
									   domain_adaption2=self.domain_adaption2)
		...
		if self.train_tagger:
			...
			if self.test_demarcated:
				# self.test_size = 0 # to just go into the test_size=0 loop in agreement_acceptor py
				...
				if type(list(result_dict)[0]) is tuple:
					with open(self.model_prefix + '_intdiff_acc.pickle', 'wb') as handle:
	...
	def vocab_ints(self, data_name='Not', save_data=False, n_examples=None):

		self.log('Loading Dataset file')
		print('Loading Dataset file')
		self.log('Creating Vocab,POS to Int and vice versa dictionaries')
		print('Creating Vocab,POS to Int and vice versa dictionaries')
		...
		# deps = deps_from_tsv(filen, limit=n_examples)
		...
		for dep in deps:
			...
			POS_tags = dep['pos_sentence'].split()

			# for adding inflected verb form to vocab to int and int to vocab dicts
			v = int(dep['verb_index']) - 1
			inflected_verbform = self.inflect_verb[tokens[v]]
			...
			if inflected_verbform not in self.vocab_to_ints:
				x = self.vocab_to_ints[inflected_verbform] = len(self.vocab_to_ints) + 1
				self.ints_to_vocab[x] = inflected_verbform
		...
		if (save_data):
			with open('data/fullGram_v2i.pkl', 'wb') as f:
				pickle.dump(self.vocab_to_ints, f)
			with open('data/fullGram_i2v.pkl', 'wb') as f:
				pickle.dump(self.ints_to_vocab, f)
			with open('data/fullGram_p2i.pkl', 'wb') as f:
				pickle.dump(self.pos_to_ints, f)
			with open('data/fullGram_i2p.pkl', 'wb') as f:
				pickle.dump(self.ints_to_pos, f)

	def load_examples(self, data_name, filen, n_examples=None):
		'''
		...
		deps = deps_from_tsv('data/' + filen, limit=n_examples)
	...
	def update_attn_dict(self, attention_weights, verb_loc, x_test):
		...
		for i in range(bsz):
			...
			with_indices.sort(key=lambda x: x[1],
							  reverse=True)  # sorting in descending order with respect to attention weights
			with_indices = [(j[0] - verb_index[i], j[1]) for j in
							with_indices]  # a list containing (location wrt verb and probablity)
	...
	def compare_models(self, m1, m2):
		...
		d = {'sent': x_diff_pred, 'ground_truth': ground_truth, 'verbLoc': verb_location, 'pred1': predicted1,
			 'pred2': predicted2}
		with open("different_sent_dump.csv", "w") as outfile:
			writer = csv.writer(outfile)
			writer.writerow(d.keys())
			writer.writerows(zip(*d.values()))
		msg = "Accuracy of Model1 is {} \t Accuracy for Model 2 is {}".format(
			float(accuracy_1) / float(example_processed), float(accuracy_2) / float(example_processed))
		print(msg)
		self.log(msg)
	...
	def validate(self):
		...
		with torch.no_grad():
			...
			for x_test, y_test, verb_loc_test in self.Test_DataGenerator:
				...
				example_processed += bsz
				if self.act_attention or self.max_attn:
					loss, _, _, acc, attention_weights, h_n, h_last = self.model.predict(x_test, y_test,
																						 compute_loss=True)
				else:
		...
		with open(self.model_prefix + '_hidden_last.pkl', 'wb') as f:
		...
		for epoch in range(self.num_epochs):
			...
			for x_batch, y_batch, _ in self.Train_DataGenerator:
				...
				if self.act_attention or self.max_attn:
					if self.use_hidden:
						if flag:
							loss, _, _, _, attention_weights, h_n, h_last = self.model.predict(x_batch, y_batch, h_n,
																							   compute_loss=True)
							flag = False
						else:
							loss, _, _, _, attention_weights, h_n, h_last = self.model.predict(x_batch, y_batch,
																							   h_n[:, 0:bsz, :],
																							   compute_loss=True)
					else:
					...
					if self.use_hidden:
						if flag:
							...
							loss, _, _, _, h_n, h_last = self.model.predict(x_batch, y_batch, h_n[:, 0:bsz, :],
																			compute_loss=True)
					else:
				...
				if (index) % 30 == 0:
					...
					self.demarcate_train = not self.demarcate_train
					counter = 0
					args = [index, total_batches, epoch, self.num_epochs, max_acc, epoch_loss / index, min_loss,
							float(index) / float(elapsed_time)]
					self.log(
						"Total bsz done {}/{} || Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f} || Speed {} b/s".format(
							*args))
					self.log_grad(
						"Total bsz done {}/{} || Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f} || Speed {} b/s".format(
							*args))
	...
	def create_model_POS(self):
		...
		print('vocab size : ' + str(len(self.vocab_to_ints)))
		self.model = POS_Tagger(rnn_arch=self.rnn_arch, embedding_size=self.embedding_size,
								hidden_size=self.hidden_size, vocab_size=len(self.vocab_to_ints) + 1,
								num_layers=self.num_layers, output_size=len(self.pos_to_ints) + 1, dropout=self.dropout,
								act_attention=self.act_attention, max_attn=self.max_attn, num_heads=self.nheads,
		...
		for epoch in range(self.num_epochs):
			...
			for x_batch, y_batch, _ in self.Train_DataGenerator:
				...
				if (index) % 30 == 0:
					...
					counter = 0
					self.log(
						"Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f}".format(
							epoch, self.num_epochs, max_acc, epoch_loss / (index * bsz), min_loss))
					self.log_grad(
						"Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f}".format(
							epoch, self.num_epochs, max_acc, epoch_loss / (index * bsz), min_loss))
					print(
			...
			print(
				"Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f}".format(
					epoch, self.num_epochs, max_acc, epoch_loss / len(self.Train_DataGenerator.dataset), min_loss))
			self.log(
				"Total Epochs Done {}/{} || Max Validation Accuracy {:.4f} || Epoch loss {:.4f} || Min loss {:.4f}".format(
					epoch, self.num_epochs, max_acc, epoch_loss / len(self.Train_DataGenerator.dataset), min_loss))
			self.log_grad(
